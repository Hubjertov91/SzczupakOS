...
void pmm_init(uint64_t mem_start, uint64_t mem_end) {
    ...
    serial_write("[PMM] kernel_end: 0x");
    serial_write_hex(kernel_end_addr);
    serial_write("\n");
    pmm_mark_used(0x100000, kernel_end_addr);
    
    serial_write("[PMM] p4_table: 0x");
    serial_write_hex((uint64_t)&p4_table);
    serial_write("\n");
    serial_write("[PMM] p3_table: 0x");
    serial_write_hex((uint64_t)&p3_table);
    serial_write("\n");
    serial_write("[PMM] p2_table: 0x");
    serial_write_hex((uint64_t)&p2_table);
    serial_write("\n");
    ...
    for (uint64_t i = 0; i < pmm_total_pages; i++) {
        if (!bitmap_test(i)) {
            serial_write("[PMM] First free page: 0x");
            serial_write_hex(pmm_start + i * PAGE_SIZE);
            serial_write("\n");
            break;
        }
    }
