...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        char path[TOUCH_PATH_MAX];
        if (to_abs_path(argv[i], path, sizeof(path)) != 0) {
            printf("touch: invalid path: %s\n", argv[i]);
            rc = 1;
            continue;
        }

        long ret = sys_fs_touch(path);
        if (ret == 0) {
            continue;
        } else if (ret == -1) {
            printf("touch: invalid path: %s\n", argv[i]);
        } else if (ret == -2) {
            printf("touch: parent directory not found: %s\n", argv[i]);
        } else if (ret == -3) {
